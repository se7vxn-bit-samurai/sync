  ...
  if(!selInfo.entries.length){mid+=`<p style="color:var(--tm);font-size:13px;margin-top:10px">No schedule data for this day.</p>`;}
    ...
    if(working.length){
      ...
      working.forEach(e=>{
        ...
        mid+=`<button class="exc-flag${hasExc?' has-exc':''}" onclick="${_excWithAgent?`openAgentDrawer('${XJS(_excWithAgent.agentName)}')`:`S._excFormOpen=S._excFormOpen==='${formKey}'?null:'${formKey}';ren()`}">${hasExc?'⚡'+personExcs.length:'+ Flag'}</button>`;
        // Swap button — opens inline swap log form prefilled with this person as absent
        const swapFormKey='swap|'+excKey(selDate)+'|'+esc;
        mid+=`<button class="dd-mini-btn" style="${isCovering||isAbsent?'color:var(--early)':''}" onclick="S._swapFormDay=S._swapFormDay==='${swapFormKey}'?null:'${swapFormKey}';ren()" title="Log shift swap for this day">⇄</button>`;
        mid+=`<div class="hc-widget"><span class="hc-label" style="font-size:8px;color:var(--tm);opacity:.5;letter-spacing:.5px;position:absolute;top:-9px;left:0;white-space:nowrap">${hc._fromRoster?'HC <span style="color:var(--accent);opacity:.8">👥</span>':'HC'}</span><input type="number" min="0" max="999" value="${hc.present||""}" placeholder="act" class="hc-inp"${inpStyle} onchange="setHc(new Date(${da}),'${esc}','present',this.value)" title="Actual headcount — agents present on shift"><span class="hc-sep">/</span><input type="number" min="0" max="999" value="${hc._fromRoster?'':hc.total||""}" placeholder="${hc._fromRoster?hc.total:'req'}" class="hc-inp"${hc._fromRoster?' style="border-color:rgba(122,180,255,.25)"':''} onchange="setHc(new Date(${da}),'${esc}','total',this.value)" title="${hc._fromRoster?'From roster: '+hc.total+' agents — type to override':'Required headcount — agents needed on shift'}"></div>`;
    ...
    if(offPpl.length){
      ...
      offPpl.forEach(e=>{
        ...
        const swapFormKey='swap|'+excKey(selDate)+'|off|'+XJ(e.name);
        ...
        if(covered){
          ...
          mid+=`<button class="dd-mini-btn" style="margin-left:auto;font-size:11px" onclick="S._swapFormDay=S._swapFormDay==='${swapFormKey}'?null:'${swapFormKey}';ren()" title="Log who covered this person">⇄ Cover</button>`;
        }
        mid+=`</div>`;
    ...
    if(working.length){
      const _hcStripAgg=working.reduce((acc,e)=>{
        ...
        acc.rostered+=(hc.total||0);acc.present+=(hc.present||0);acc.filled+=(hc.total>0||hc.present>0?1:0);
        return acc;
