...
function rRaw(el){
  // A resumed session has entries but no workbook behind them, so there are no
  // sheets to show and nothing here can be read.
  if(!S.wb||!S.shs||!S.shs.length){
    el.innerHTML=`<div class="empty">Raw sheet data is only available for a schedule loaded from a file in this session. Re-import the workbook to inspect its sheets.</div>`;
    return;
  }
