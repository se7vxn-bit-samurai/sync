function rPeopleAgentsView(){
  ...
  if(addLeaders.length){
    ...
    h+=`<div style="display:flex;align-items:center;gap:8px;margin:8px 0 6px"><span style="font-size:10px;color:var(--tm);font-weight:700;text-transform:uppercase">Leader</span><select onchange="S._agentsBulkTL=this.value;rPeople($('ca'))" style="padding:4px 8px;border:1px solid var(--bdr);border-radius:6px;background:var(--bg);color:var(--text);font-family:inherit;font-size:11px">${addLeaders.map(l=>`<option value="${XJS(l)}"${S._agentsBulkTL===l?" selected":""}>${X(l)}</option>`).join("")}</select></div>`;
...
function rPeople(el){
  ...
  if(!["dashboard","team","agents","logbook","events"].includes(S.peopleSubTab))S.peopleSubTab="dashboard";
