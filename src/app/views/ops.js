...
function _opsSourceRows(){
  ...
  if(!sheets.length&&S.raw)sheets.push({name:"Parsed schedule rows",rows:S.raw.length,type:"parsed"});
  return sheets;
}
...
function rOps(el){
  ...
  const blueprint=ctx.health?ctx.health.grade+" "+ctx.health.avg:"--";
  ...
  if(view==="overview"){
    body='<div class="sd-kpis" style="margin:0">'+
      ...
      _opsMetric("Blueprint",blueprint,(S.shs||[]).length+" sources")+
    '</div><div class="qol-two-col" style="margin-top:12px"><section class="an-card"><div class="an-card-hd"><h4>Decision Queue</h4><span>Ops Hub port</span></div><div class="an-card-body">'+_opsIssueRows(5)+'</div></section><section class="an-card"><div class="an-card-hd"><h4>Sources Hub</h4><span>'+_opsSourceRows().length+' sources</span></div><div class="an-card-body"><div class="sd-list">'+_opsSourceCards()+'</div></div></section><section class="an-card"><div class="an-card-hd"><h4>Logbook</h4><span>people memory</span></div><div class="an-card-body">'+_opsLogbookRows(5)+'</div></section><section class="an-card"><div class="an-card-hd"><h4>Events</h4><span>current scope</span></div><div class="an-card-body">'+_opsEventRows(5)+'</div></section></div>';
  }else if(view==="decisions"){
    body='<section class="an-card"><div class="an-card-hd"><h4>Decision Queue</h4><span>'+ctx.issues.length+' open</span></div><div class="an-card-body"><div class="qol-filterbar"><span style="font-size:11px;color:var(--tm)">Ops Hub pattern: every signal should have owner, evidence, status, and next action.</span><button class="qol-row-btn primary" style="margin-left:auto" onclick="openIssueInbox()">Open Issue Inbox</button></div>'+_opsIssueRows(80)+'</div></section>';
  }else if(view==="sources"){
    body='<section class="an-card"><div class="an-card-hd"><h4>Sources Hub</h4><span>'+_opsSourceRows().length+' loaded</span></div><div class="an-card-body"><div class="qol-filterbar"><span style="font-size:11px;color:var(--tm)">Port target: confidence, authority, row count, date range, and conflict status for every file.</span><button class="qol-row-btn primary" style="margin-left:auto" onclick="openImportReview()">Import Review</button></div><div class="sd-list">'+_opsSourceCards()+'</div></div></section>';
    ...
    body='<section class="an-card"><div class="an-card-hd"><h4>Events Stream</h4><span>absence / OT / acting / coaching</span></div><div class="an-card-body"><div class="qol-filterbar"><span style="font-size:11px;color:var(--tm)">Ops Hub pattern: upcoming operational moments in one sortable stream.</span><button class="qol-row-btn primary" style="margin-left:auto" onclick="railNavPeople(&quot;events&quot;)">People Events</button></div>'+_opsEventRows(140)+'</div></section>';
  }else{
  ...
  el.innerHTML='<div class="people-hq ops-hq">'+_opsSubtabNav(view)+'<div class="people-context"><span class="ctx-label">Ops scope</span><span class="ctx-pill">'+X(S.activeDept||"Current department")+'</span><span class="ctx-label">Month</span><span class="ctx-pill">'+X(monthLabel()||"--")+'</span><span class="ctx-label">Port map</span><span class="ctx-pill">Decision Queue - Sources Hub - Logbook - Events - Handoff</span></div><div style="flex:1;overflow:auto;padding:12px">'+body+'</div></div>';
}