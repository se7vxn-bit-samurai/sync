    ...
    if(mode==="roster"){
      ...
      if(ddHcEntered){
        ...
        ddWork.forEach(e=>{
          ...
          h+=`<div class="da-an-row"><span class="dar-l" style="cursor:pointer" onclick="navToPerson('${XJS(e.name)}')">${X(e.name.split(" ")[0])}</span><span style="font-size:11px;font-family:'JetBrains Mono',monospace">${hc.present||"—"}/${hc.total||"—"}${pct2!==null?` <span class="${pctCls2}">(${pct2}%)</span>`:""}</span></div>`;
        });
      } else {
