...
function scanFileQuick(wb,filename,manualOverride){
  ...
  try{
    ...
    const seen={};const entries=[];const nameSheetMap={};const parsedSheets=[];const birthdayDirectory=[];
    shs.forEach(shName=>{
      const ws=wb.Sheets[shName];if(!ws)return;
      const data=sheetToRosterAOA(ws);
      if(isBirthdayDirectorySheet(data)){
        const recs=parseBirthdayDirectory(data);
        recs.forEach(rec=>birthdayDirectory.push(Object.assign({sheet:shName},rec)));
        S.parseInfo.push({sheet:shName,parser:"birthdayDirectory",confidence:"high",reason:`Birthday directory: ${recs.length} people found, not a schedule sheet`,count:0,warnings:[]});
        return;// not a schedule sheet — don't run it through autoParse
      }
    ...
    // Birthday-directory names: dedupe, then split into people already known (schedule or
    // People registry) vs. unlinked ones needing a role + reports-to decision before load.
    const birthdayByName=new Map();
    birthdayDirectory.forEach(b=>{if(!birthdayByName.has(b.name))birthdayByName.set(b.name,b);});
    const knownPeopleLower=new Set(Object.keys(S.people||{}).map(n=>n.toLowerCase()));
    const namesLower=new Set(names.map(n=>n.toLowerCase()));
    const unlinkedPeople=[...birthdayByName.values()].filter(b=>!knownPeopleLower.has(b.name.toLowerCase())&&!namesLower.has(b.name.toLowerCase()));
    if(unlinkedPeople.length){
      const unlinkedLower=new Set(unlinkedPeople.map(b=>b.name.toLowerCase()));
      ghostAgents=ghostAgents.filter(g=>!unlinkedLower.has(g.name.toLowerCase()));
    }
    // A schedule sheet parsed by parseRotatingRoleCalendar has no real name column — its
    // entries are keyed to the sheet name itself. Surface that as an identity to resolve
    // too, instead of silently keeping the sheet title as a fake person.
    const roleSheetNames=new Set(parseInfo.filter(p=>p.parser==="parseRotatingRoleCalendar").map(p=>p.sheet));
    const unresolvedRoleIdentities=names.filter(n=>roleSheetNames.has(n));
    ...
    return{entries,names,months,work:work.length,off:off.length,total:entries.length,
      ...
      birthdayDirectory:[...birthdayByName.values()],unlinkedPeople,unresolvedRoleIdentities};
  }finally{
...
// ── Birthday / people-directory sheet: a flat Month|Date|Name (or similar) list that isn't
// a schedule at all. Common in real workbooks as a reference tab with no role/team-leader
// info, so it needs cross-referencing against known people rather than being parsed as a
// rota or silently ignored.
const BIRTHDAY_MONTH_HEADERS=/^month$/i,BIRTHDAY_DATE_HEADERS=/^(date|day)$/i,BIRTHDAY_NAME_HEADERS=/^(agent|name|person|employee|team\s*member)$/i;
function _birthdayDirectoryCols(data){
  const header=(data&&data[0]||[]).map(v=>String(v||"").trim().toLowerCase());
  const monthCol=header.findIndex(h=>BIRTHDAY_MONTH_HEADERS.test(h));
  const dateCol=header.findIndex(h=>BIRTHDAY_DATE_HEADERS.test(h));
  const nameCol=header.findIndex(h=>BIRTHDAY_NAME_HEADERS.test(h));
  return{monthCol,dateCol,nameCol};
}
function isBirthdayDirectorySheet(data){
  if(!data||data.length<2)return false;
  const{monthCol,dateCol,nameCol}=_birthdayDirectoryCols(data);
  if(monthCol<0||dateCol<0||nameCol<0)return false;
  let lastMonth="",checked=0,hits=0;
  for(let r=1;r<Math.min(data.length,40);r++){
    const row=data[r]||[];
    if(!row.some(v=>String(v||"").trim()))continue;
    checked++;
    const mv=String(row[monthCol]||"").trim();if(mv)lastMonth=mv;
    const dv=String(row[dateCol]||"").trim();
    const nv=String(row[nameCol]||"").trim();
    const monthOk=!lastMonth||isMonthLabel(lastMonth);
    const dayOk=/^\d{1,2}$/.test(dv)&&+dv>=1&&+dv<=31;
    const nameOk=nv.length>=2&&!/\d/.test(nv)&&!isPersonNoiseLabel(nv);
    if(monthOk&&dayOk&&nameOk)hits++;
  }
  return checked>=2&&hits/checked>=0.6;
}
function parseBirthdayDirectory(data){
  const{monthCol,dateCol,nameCol}=_birthdayDirectoryCols(data);
  const out=[];let lastMonth="";
  for(let r=1;r<(data||[]).length;r++){
    const row=data[r]||[];
    const mv=String(row[monthCol]||"").trim();if(mv)lastMonth=mv;
    const dv=String(row[dateCol]||"").trim();
    const nv=cleanPersonName(String(row[nameCol]||"").trim());
    if(!nv||!/^\d{1,2}$/.test(dv))continue;
    const monthIdx=MOFULL.findIndex(mn=>mn.toLowerCase()===lastMonth.toLowerCase());
    if(monthIdx<0)continue;
    const day=+dv;if(day<1||day>31)continue;
    out.push({name:nv,month:monthIdx,day,monthDay:P(monthIdx+1)+"-"+P(day)});
  }
  return out;
}
...
function showParsePreview(wb,filename,manualOverride,sourceMeta,opts){
  ...
  // Does this workbook's detected name land on a project that already exists? If so the default
  // "replace" is a merge into real, existing data — the one import outcome that is genuinely hard
  // to unpick afterwards — so the copy route has to be offered here, at the decision point, rather
  // than left as something to notice afterwards. See _switchPendingLoadToCopy.
  const collidingProject=(()=>{
    try{
      const detected=String(_pendingLoad.departmentName||scan.deptName||"").trim().toLowerCase();
      if(!detected||loadMode==="add")return null;
      const list=typeof nsListCanonicalProjects==="function"?nsListCanonicalProjects():[];
      return list.find(p=>String(p.name||"").trim().toLowerCase()===detected)||null;
    }catch(err){return null;}
  })();
  _pendingLoad.collidingProject=collidingProject;
  ...
  if(hasCache){
    ...
    h+=_parseCopyNoticeHtml(collidingProject);
...
// Shown only when the incoming workbook's detected name matches a project that already exists —
// the case where "Load" quietly means "merge into six months of existing roster". Saying so, with
// the copy route one click away, is the difference between an informed update and an accident.
function _parseCopyNoticeHtml(project){
  if(!project)return"";
  const counts=[project.peopleCount?project.peopleCount+" people":"",project.scheduleCount?project.scheduleCount+" schedule rows":""].filter(Boolean).join(" · ")||"no data yet";
  return`<div id="parseCopyNotice" style="margin:10px 16px 0;padding:10px 12px;border:1px solid var(--wknd,#f5a623);border-radius:8px;background:rgba(245,166,35,.08)">
    <div style="font-size:12px;font-weight:600;color:var(--text);margin-bottom:3px">A project called “${X(project.name)}” already exists</div>
    <div style="font-size:11px;color:var(--tm);line-height:1.5;margin-bottom:8px">It currently holds ${X(counts)}. Loading will update that project in place. Import as a separate copy instead if this workbook is a variant, a different period, or you are not sure.</div>
    <button type="button" onclick="_switchPendingLoadToCopy()" style="padding:6px 12px;border:1px solid var(--bdr);border-radius:7px;background:var(--al);color:var(--accent);font-family:inherit;font-size:12px;cursor:pointer">Open as a copy instead</button>
  </div>`;
}
// Flips the pending import from "replace that project" to "create a new one". confirmParsePreview
// already routes mode==="add" through _nextAvailableDepartmentName, so the copy gets a free name
// ("Ops (2)") and the existing project is left untouched.
function _switchPendingLoadToCopy(){
  if(!_pendingLoad)return;
  _pendingLoad.mode="add";
  const existing=_pendingLoad.collidingProject;
  _pendingLoad.collidingProject=null;
  const notice=document.getElementById("parseCopyNotice");
  const copyName=typeof _nextAvailableDepartmentName==="function"?_nextAvailableDepartmentName(_pendingLoad.departmentName||_pendingLoad.scan.deptName):"";
  if(notice){
    notice.style.borderColor="var(--accent)";
    notice.style.background="var(--al)";
    notice.innerHTML=`<div style="font-size:12px;font-weight:600;color:var(--text);margin-bottom:3px">Importing as a separate copy</div>
      <div style="font-size:11px;color:var(--tm);line-height:1.5">${X(existing?existing.name:"The existing project")} stays exactly as it is${copyName?`. This workbook loads as “${X(copyName)}”`:""}.</div>`;
  }
  toast("Will import as a new project","ok");
}
function confirmParsePreview(){
  ...
  // Undo above covers this session; the snapshot covers "that import was wrong" discovered later.
  // Not awaited on purpose: nsSaveSnapshot clones the canonical model synchronously before its
  // first await, so the capture is of pre-import state regardless, and a slow IndexedDB write must
  // never sit between the user pressing Load and the import starting.
  if(typeof _syncAutoSnapshot==="function")_syncAutoSnapshot("Before importing "+filename,"auto-import");
