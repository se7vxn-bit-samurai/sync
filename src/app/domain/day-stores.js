...
// `recorded` distinguishes three real states: attendance not yet recorded for the day
// (default/roster fallback, recorded:false) vs. explicitly recorded — whether that's a
// genuine zero (present:0, recorded:true) or a nonzero complete count (recorded:true).
function getHc(date,name){
  const explicit=S.hc[hcKey(date,name)];
  if(explicit)return{...explicit,recorded:true};
  // Fall back to default total (present is always day-specific)
  const def=S.hc[hcDefaultKey(name)];
  if(def&&def.total)return{total:def.total,present:0,recorded:false};
  // v44.3: Fall back to roster agent count if available
  const rosterCount=getAgentCount(name);
  if(rosterCount>0)return{total:rosterCount,present:0,recorded:false,_fromRoster:true};
  return{total:0,present:0,recorded:false};
}
