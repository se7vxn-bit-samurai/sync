...
function buildSheetPeople(wb){
  ...
  // Leaders section
  ws.addRow(['Leader','Team','Role','Agents','Span','Contract Hrs','Source']).eachCell(c=>{sC(c,{bold:true,color:'FFFFFFFF',fill:ORDO.TITLE,border:ORDO.BORDER});});
  const leaders=getLeaders();
  leaders.forEach(leader=>{
    const ac=getAgentCount(leader.name);
    const soc=getSpanOfControl(leader.name);
    ws.addRow([leader.name,leader.team||'',leader.role,ac,soc.ratio,leader.contractHours||'',leader.source||'']).eachCell(c=>{sC(c,{border:ORDO.BORDER});});
  });
  ws.addRow([]);ws.addRow([]);
  // Agents section
  ...
  if(agents.length){
    ws.addRow(['Agent','Team Leader','Team','Skills','Contract','Hours','FTE','Start Date']).eachCell(c=>{sC(c,{bold:true,color:'FFFFFFFF',fill:ORDO.TITLE,border:ORDO.BORDER});});
    agents.forEach(agent=>{
      ws.addRow([agent.name,agent.teamLeader||'',agent.team||'',(agent.skills||[]).join(', '),agent.contractType||'',agent.contractHours||'',agent.fte||1,agent.startDate||'']).eachCell(c=>{sC(c,{border:ORDO.BORDER});});
    });
  }
