...
function renderToolbar(){
  ...
  h+=`<button class="sb-icon-btn sb-today${S.hlToday?' active':''}" onclick="setTodayHighlight(!S.hlToday);renderToolbar()" title="${S.hlToday?'Today highlight ON':'Today highlight OFF'}">◉</button>`;
  ...
  // ── Scratchpad / Notes ──
  const spLines=(S.scratchpad||'').trim().split(/\n/).filter(l=>l.trim());
  const spCount=spLines.length;
  h+=`<div class="sb-popover"><button class="sb-icon-btn${S._scratchOpen?' active':''}" onclick="S._scratchOpen=!S._scratchOpen;renderToolbar()" title="Quick notes">📝${spCount>0?'<span class="sb-badge">'+spCount+'</span>':''}</button>`;
  if(S._scratchOpen){
    h+=`<div class="sb-scratchpad-pop">`;
    h+=`<div class="sp-hd"><h4>Notes</h4><button onclick="S._scratchOpen=false;renderToolbar()" style="background:none;border:none;color:var(--tm);cursor:pointer;font-size:13px">✕</button></div>`;
    h+=`<textarea id="scratchInput" placeholder="Quick notes, reminders, todos...\n\nPersisted across sessions." oninput="S.scratchpad=this.value;_saveScratchpad()">${X(S.scratchpad||'')}</textarea>`;
    h+=`<div class="sp-ft">Auto-saved locally</div></div>`;
  }
  h+=`</div>`;

  // ── Filters button ──
  const hasFilters=(S.shs.length>1)||(teams.length>1);
  const activeFilterCount=(S.sh!=="__all__"&&S.shs.length>1?1:0)+(S.team!=="all"&&teams.length>1?1:0)+(S.allMonths?1:0);
  if(hasFilters){
    h+=`<div class="sb-popover"><button class="sb-icon-btn${activeFilterCount?' active':''}" onclick="S._filtersOpen=!S._filtersOpen;renderToolbar()" title="Filters${activeFilterCount?' ('+activeFilterCount+' active)':''}"><svg width="13" height="13" viewBox="0 0 14 14" fill="none"><path d="M1 2h12M3 7h8M5 12h4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>${activeFilterCount?'<span class="sb-badge">'+activeFilterCount+'</span>':''}</button>`;
    if(S._filtersOpen){
      h+=`<div style="position:absolute;top:100%;right:0;margin-top:6px;padding:14px;background:var(--bg2,rgba(10,14,26,.97));border:1px solid var(--rule,var(--bdr));border-radius:12px;box-shadow:0 12px 40px rgba(0,0,0,.35);z-index:200;min-width:220px">`;
      if(S.shs.length>1)h+=`<div style="margin-bottom:12px"><div style="font-size:10px;color:var(--ink3,var(--tm));margin-bottom:4px;text-transform:uppercase;letter-spacing:.5px;font-weight:600">Sheet</div><select onchange="S._filtersOpen=false;swSh(this.value)" style="width:100%;padding:6px 10px;border:1px solid var(--rule,var(--bdr));border-radius:8px;font-size:11px;background:var(--bg);color:var(--ink,var(--text));font-family:inherit"><option value="__all__"${S.sh==='__all__'?' selected':''}>All sheets</option>${S.shs.map(s=>`<option value="${X(s)}"${s===S.sh?' selected':''}>${X(s)}</option>`).join('')}</select></div>`;
      if(teams.length>1)h+=`<div style="margin-bottom:12px"><div style="font-size:10px;color:var(--ink3,var(--tm));margin-bottom:4px;text-transform:uppercase;letter-spacing:.5px;font-weight:600">Team</div><select onchange="S._filtersOpen=false;setTeam(this.value)" style="width:100%;padding:6px 10px;border:1px solid var(--rule,var(--bdr));border-radius:8px;font-size:11px;background:var(--bg);color:var(--ink,var(--text));font-family:inherit"><option value="all">All teams</option>${teams.map(t=>`<option value="${X(t)}"${t===S.team?' selected':''}>${X(t)}</option>`).join('')}</select></div>`;
      h+=`<label style="font-size:11px;display:flex;align-items:center;gap:8px;cursor:pointer;color:var(--ink2,var(--text))"><input type="checkbox" ${S.allMonths?'checked':''} onchange="S.allMonths=this.checked;S._filtersOpen=false;rerenderCurrentSurface();rerenderChromeOnly()" style="accent-color:var(--accent)"> All months view</label></div>`;
    }
    h+=`</div>`;
  }
...
function shuffleDashboardColorway(button){
  ...
  window.setTimeout(()=>shuffleThemeColorway(),700);
}
...
function renderChrome(){renderMonthNav();renderToolbar();renderTabs();renderKeyboardHints();renderInfoBar();renderContinuityBar();renderRailToggle();}
...
function _renderNow(){normalizeTabState();renderChrome();renderActiveTab();}
