...
// Escapes for a single-quoted JS string that may itself sit inside a double-quoted HTML attribute
// (onclick="fn('${XJ(v)}')"). \uXXXX escapes are decoded by JS but never by the HTML parser, so a
// value can neither end the attribute (") nor smuggle a quote in as an entity (&#39;).
function XJ(s){return String(s).replace(/[\\'"&<>\r\n\u2028\u2029]/g,c=>"\\u"+c.charCodeAt(0).toString(16).padStart(4,"0"));}
...
function pDt(v){
  ...
  if(typeof v==="number"){
    // Only treat as an Excel date serial within a plausible range (~1954-2091).
    // Bare small integers (day-of-month numbers, counts) must NOT resolve to
    // phantom dates near the 1900 epoch — mirrors the string-path bound below.
    if(v>=20000&&v<=70000){
